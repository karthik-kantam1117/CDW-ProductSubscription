  ...
  while (productLineItems.hasNext()) {
    ...
    if (productLineItem.optionProductLineItem) {
      ...
      if (!adjustments.isEmpty()) {
        ...
        var adjustmentPrice = price
          .multiply(quantity2.value)
          .multiply(-1.0)
          .add(adjustedPrice);
    ...
    if (
      ...
      var subscriptionPrice = subscriptionHelpers.getSubscriptionPrice(productLineItem.product);
      if (!empty(subscriptionPrice)) {
        productLineItem.setPriceValue(subscriptionPrice.valueOrNull);
      }
    }
  }
}
...
/**
 * This method returns subscription price associated with the product
 * @param {*} productLineItem
 * @returns
 */
function getSubscriptionPrice(productLineItem) {
  var subscriptionPrice;
  if (
    productLineItem &&
    productLineItem.product &&
    productLineItem.product.priceModel
  ) {
    subscriptionPrice = productLineItem.product.priceModel.getPriceBookPrice(
...
exports.calculateTax = function (basket) {
  ...
  if (
    !basket.getPriceAdjustments().empty ||
    !basket.getShippingPriceAdjustments().empty
  ) {
    if (
      collections.first(
        basket.getPriceAdjustments(),
        function (priceAdjustment) {
          return taxesMap[priceAdjustment.UUID] === null;
        }
      ) ||
      collections.first(
        basket.getShippingPriceAdjustments(),
        function (shippingPriceAdjustment) {
      ...
      var basketPriceAdjustmentsTaxRate =
        (basket.getMerchandizeTotalGrossPrice().value +
          basket.getShippingTotalGrossPrice().value) /
          (basket.getMerchandizeTotalNetPrice().value +
            basket.getShippingTotalNetPrice()) -
        1;

      var basketPriceAdjustments = basket.getPriceAdjustments();
      collections.forEach(
        basketPriceAdjustments,
        function (basketPriceAdjustment) {
      ...
      collections.forEach(
        basketShippingPriceAdjustments,
        function (basketShippingPriceAdjustment) {
          basketShippingPriceAdjustment.updateTax(
            totalShippingGrossPrice / totalShippingNetPrice - 1
          );
        }
      );
    }
